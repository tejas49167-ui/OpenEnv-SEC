...
class BaseGrader(ABC):
    ...
    def efficiency_credit(self, state: EnvironmentState) -> float:
        ...
        return clamp_open_unit_interval(1.0 - (max(0, state.steps_taken - 1) / state.max_steps))
    ...
    def build_reward(
        ...
        return Reward(
            ...
            step_score=clamp_open_unit_interval(components.get("step", 0.0)),
            terminal_score=clamp_open_unit_interval(components.get("terminal", 0.0)),
            investigation_score=clamp_open_unit_interval(components.get("investigation", 0.0)),
            decision_score=clamp_open_unit_interval(components.get("decision", 0.0)),
            efficiency_score=clamp_open_unit_interval(components.get("efficiency", 0.0)),
